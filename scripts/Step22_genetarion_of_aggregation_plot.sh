...
for subset in ES_active_E_intergenic ES_E_intergenic ES_non_active_E_intergenic Strand_ES_active_E_intragenic Strand_ES_E_intragenic Strand_ES_non_active_E_intragenic; do
    ...
    # Matrix of ATAC, histon signal generation
    computeMatrix reference-point --referencePoint center -a 5000 -b 5000 -R "$summit_bed" \
    -S "$bw_dir/ATAC.bw" "$bw_dir/H3K27ac.bw" "$bw_dir/H3K4me1.bw" --missingDataAsZero -p "$THREADS" -o "$matrix_out"

    # Aggregation plot generation
    plotProfile -m "$matrix_out" -out "$profile_out" --plotHeight 8 --plotWidth 8 --yMax 6 \
    --perGroup --refPointLabel "summit" --regionsLabel "${subset}" --samplesLabel ATAC H3K27ac H3K4me1 --colors '#2ca339' '#e7211a' '#f4da26'
